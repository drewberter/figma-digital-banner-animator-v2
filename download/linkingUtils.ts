...
export function syncGifFrameLayerVisibility(
  ...
  try {
    ...
    const updateVisibilityRecursively = (
      ...
      const countVisibleLayers = (layers: AnimationLayer[]): number => {
        return layers.reduce((count, l) => {
          ...
          const isVisible = !hiddenLayers.includes(l.id);
    ...
    // Define framesWithSameNumber outside the try block so it's accessible throughout
    let framesWithSameNumber: GifFrame[] = [];
    
    // Check if we can use LinkRegistry for finding frames with the same frame number
    try {
      // Import the LinkRegistry here to avoid circular dependencies 
      // Dynamic import to prevent circular dependencies
      const linkRegistryModule = require('./linkRegistry');
      const linkRegistry = linkRegistryModule.linkRegistry;
      
      // Use the LinkRegistry method to find frames with the same number
      const linkedFrameIds = linkRegistry.findFramesByNumber(frameNumber);
      
      // Filter out the source frame and any frames not in our current frames list
      const validLinkedFrameIds = linkedFrameIds.filter((id: string) => id !== frameId);
      
      // Map IDs to actual frame objects
      const linkedFrames = validLinkedFrameIds.map((id: string) => {
        const frame = updatedFrames.find((f: GifFrame) => f.id === id);
        return frame;
      }).filter(Boolean) as GifFrame[];
      
      if (linkedFrames.length > 0) {
        syncDebug(`LinkRegistry found ${linkedFrames.length} linked frames for frame number ${frameNumber}`);
        // Use the frames found from the registry
        framesWithSameNumber = linkedFrames;
        syncDebug(`Using LinkRegistry: Found ${framesWithSameNumber.length} frames with frame number ${frameNumber}`);
      } else {
        // Fall back to the existing method if registry doesn't have linked frames
        syncDebug(`LinkRegistry didn't find any linked frames, falling back to frame filtering method`);
        throw new Error("No linked frames in registry");
      }
    } catch (error) {
      // Fallback to the original method if LinkRegistry is not available or has no linked frames
      ...
      framesWithSameNumber = updatedFrames.filter((f: GifFrame) => {
      ...
      syncDebug(`Fallback method: Found ${framesWithSameNumber.length} other frames with frame number ${frameNumber} from different ad sizes`);
    }
    ...
    framesWithSameNumber.forEach((frame: GifFrame) => {
      ...
      // STRATEGY 1: Enhanced name-based layer matching
      if (sourceLayerName && frame.layers) {
        // CRITICAL FIX: Try to use LinkRegistry to find linked layers by name first
        try {
          // Import the LinkRegistry here to avoid circular dependencies 
          const linkRegistryModule = require('./linkRegistry');
          const linkRegistry = linkRegistryModule.linkRegistry;
          
          // First find if this layer is registered in the linkRegistry
          if (sourceLayer && sourceLayer.id) {
            // Get all linked layers for this layer in GIF mode
            const linkedLayerIds = linkRegistry.getLinkedLayers(sourceLayer.id, 'gif');
            
            if (linkedLayerIds && linkedLayerIds.length > 0) {
              syncDebug(`LinkRegistry: Found ${linkedLayerIds.length} linked layers for ${sourceLayer.id}`);
              
              // Find a linked layer that exists in the current target frame
              const targetLayerFromRegistry = linkedLayerIds.find((id: string) => {
                // Find layer in this frame by recursive search
                const layerExists = (layers: AnimationLayer[]): boolean => {
                  for (const layer of layers) {
                    if (layer.id === id) return true;
                    if (layer.children && layer.children.length) {
                      if (layerExists(layer.children)) return true;
                    }
                  }
                  return false;
                };
                
                return layerExists(frame.layers);
              });
              
              if (targetLayerFromRegistry) {
                syncDebug(`LinkRegistry: Using linked layer ${targetLayerFromRegistry} from registry`);
                
                // Find the actual layer by ID
                const findLayerById = (layers: AnimationLayer[], id: string): { layer: AnimationLayer | null, path: string[] } => {
                  const find = (layers: AnimationLayer[], id: string, currentPath: string[] = []): { layer: AnimationLayer | null, path: string[] } => {
                    for (const layer of layers) {
                      if (layer.id === id) {
                        return { layer, path: [...currentPath, layer.id] };
                      }
                      if (layer.children && layer.children.length) {
                        const result = find(layer.children, id, [...currentPath, layer.id]);
                  ...
                  return find(layers, id);
                };
                
                const { layer: registryLayer, path: registryPath } = findLayerById(frame.layers, targetLayerFromRegistry);
                
                if (registryLayer) {
                  targetLayer = registryLayer;
                  targetLayerId = registryLayer.id;
                  targetPath = registryPath;
                  syncDebug(`LinkRegistry: Found layer ${registryLayer.name} (${registryLayer.id}) in frame ${frame.id}`);
                }
              }
            }
          }
        } catch (error) {
          syncWarn(`Error using LinkRegistry: ${error}`);
        }
        
        // Fall back to name-based matching if LinkRegistry didn't find anything
        if (!targetLayer) {
          syncDebug(`Falling back to name-based matching for "${sourceLayerName}"`);
          
          // Define an enhanced function to find all layers with the same name
          // regardless of their position in the layer hierarchy
          const findAllLayersWithSameName = (
            layers: AnimationLayer[],
            targetName: string,
            currentPath: string[] = []
          ): Array<{ layer: AnimationLayer; path: string[] }> => {
            const results: Array<{ layer: AnimationLayer; path: string[] }> = [];
            
            // Helper function to recursively process layers
            const processLayer = (layer: AnimationLayer, path: string[]) => {
              // IMPORTANT: For GIF frame mode, we need to match by name only, ignoring case
              // This is a critical fix for cross-ad-size linking
              if (layer.name && layer.name.toLowerCase() === targetName.toLowerCase()) {
                results.push({ layer, path });
                syncDebug(`Found layer with matching name "${targetName}" at path: ${path.join(' > ')}`);
              }
              
              // Process children regardless of whether this layer matched
              if (layer.children && Array.isArray(layer.children)) {
                layer.children.forEach(child => {
                  processLayer(child, [...path, child.id]);
                });
              }
            };
            ...
            for (const layer of layers) {
              processLayer(layer, [...currentPath, layer.id]);
            }
            
            return results;
          };
          
          // Find all layers with matching name in the target frame
          const matchingLayers = findAllLayersWithSameName(frame.layers, sourceLayerName);
          
          if (matchingLayers.length > 0) {
            // Log all matches for debugging
            syncDebug(`Found ${matchingLayers.length} layers named "${sourceLayerName}" in frame ${frame.id}`);
            
            if (matchingLayers.length > 1) {
              matchingLayers.forEach((match, index) => {
                syncDebug(`Match #${index + 1}: Layer ${match.layer.id} at path: ${match.path.join(' > ')}`);
              });
            }
            
            // Prefer the first match for simplicity
            // In a more sophisticated version, we could use heuristics to find the "best" match
            const bestMatch = matchingLayers[0];
            targetLayer = bestMatch.layer;
            targetLayerId = targetLayer.id;
            targetPath = bestMatch.path;
            
            syncDebug(`Using layer "${targetLayer.name}" (${targetLayerId}) for syncing`);
          } else {
            syncWarn(`No layers with name "${sourceLayerName}" found in frame ${frame.id}`);
          }
        }
      }
