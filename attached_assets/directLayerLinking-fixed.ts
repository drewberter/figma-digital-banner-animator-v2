    ...
    updatedFrames.forEach((frame: GifFrame, frameIndex: number) => {
      ...
      if (matchingLayers.length > 0) {
        ...
        matchingLayers.forEach(layer => {
          ...
          // SPECIAL HANDLING: For background layer, invert the visibility state
          ...
          // Update the layer's visible property for consistency (used in the UI)
          updateLayerVisibility(frame.layers, layer.id, actualVisibility);
...
export function setLayerVisibility(
  ...
  // Update the layer's visible property for consistency
  if (updatedFrame.layers) {
    updateLayerVisibility(updatedFrame.layers, layerId, visible);
  }
