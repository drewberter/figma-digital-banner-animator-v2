...
export function syncLayersByNameSimple(
  ...
  try {
    ...
    if (isBackgroundLayer) {
      ...
      log(`Background layer identified, but no special handling - visibility state no longer inverted`);
    }
    ...
    updatedFrames.forEach((frame: GifFrame, frameIndex: number) => {
      ...
      if (matchingLayers.length > 0) {
        ...
        matchingLayers.forEach(layer => {
          ...
          // NOTE: We previously inverted visibility for background layers, but this was causing test failures
          // Per user requirements, background layers now behave the same as regular layers
          const actualVisibility = makeVisible; // No special handling for background layers
